...
// bzw. Staedte), und mit Auswahl die einzelnen Haeuser. Tabellen
 ...
 * Ohne Rang (stillgelegtes Haus, fehlender Wert) ein Gedankenstrich —
...
SELECT m.betrieb                                  AS "Betrieb",
  ...
  -- LEFT JOIN, nicht JOIN: sieben laufende Haeuser haben keine
 ...
 ORDER BY m.betrieb, m.reihenfolge`,
  },
  {
    ...
    beschreibung:
      'Die Umsatzveränderung gegenüber dem jeweiligen Vorjahresmonat über zwei Jahre: dieses Haus, seine Marke und seine Stadt in einem Bild. Laufen alle drei Linien gemeinsam nach unten, ist der Rückgang keine Aussage über dieses Haus.\n\nOhne gewählten Betrieb stehen hier die Marken — drei Linien je Betrieb wären bei 56 Häusern keine Kurve mehr.',
    anzeige: 'line',
    parameter: [P_MONAT, P_BETRIEB],
    // Festes 24-Monats-Fenster am Monatsfilter statt des Zeitraumfilters
    // der Seite: die Karte liest DREI Tabellen (Haus, Marke, Stadt), und
...
SELECT v.bereich_name                             AS "Kennzahl",
 ...
 ORDER BY v.reihenfolge, v.abweichung NULLS LAST`,
  },
  {
    ...
    beschreibung:
      'Die Umsatzveränderung gegenüber dem jeweiligen Vorjahresmonat, über die letzten zwei Jahre bis zum gewählten Monat. Läuft die Markenlinie mit nach unten, ist es kein Problem dieses Hauses.\n\nOhne Auswahl oben stehen hier die Marken selbst — 22 Häuser gleichzeitig wären keine lesbare Kurve.',
    anzeige: 'line',
    parameter: [P_MONAT, P_BETRIEB],
...
SELECT x."Monat", x."Reihe", x."Umsatz % ggü. Vorjahr"
 ...
 ORDER BY x.sortierung, x."Monat"`,
  ...
  },
  {
    ...
    name: 'Alle Häuser der Marke',
    beschreibung:
      'Die Häuser derselben Marke im gewählten Monat, das stärkste zuerst. Zeigt auf einen Blick, ob ein Rückgang die ganze Marke betrifft oder ein einzelnes Haus. Ein ◀ markiert den oben gewählten Betrieb.\n\nHäuser ohne laufenden Umsatz stehen nicht in der Liste — sie würden mit −100 % jede Zeile daneben verzerren.',
    anzeige: 'table',
    parameter: [P_MONAT, P_BETRIEB],
    sql: `${MONAT_CTE}
, bezug AS (
...
SELECT ${MARKIERUNG}                              AS "◀",
 ...
 ORDER BY coalesce(r.konzept, '(nicht zugeordnet)'), r.umsatz_ist DESC NULLS LAST`,
  ...
  // und die absoluten Quoten dahinter. Die Haeuser einer Stadt gehoeren
  ...
  // Haus in seiner eigenen Einheit.
  // ===================================================================
  {
    schluessel: 'vs_kopf',
    name: 'Der Betrieb und seine Stadt',
    beschreibung:
      'Wo der Betrieb steht, wie viele Häuser der Gruppe dort sonst noch stehen und wie sich die Stadt insgesamt entwickelt hat. Steht hier „(keine Stadt hinterlegt)“, bleiben die Karten darunter leer — der Betrieb fehlt dann in der Standortliste, und wer noch fehlt, steht ganz unten auf dieser Seite.\n\n„Häuser mit Umsatz“ sind die, die im gewählten Monat Geschäft gemacht haben; „geführt“ zählt auch stillgelegte mit.',
    anzeige: 'table',
    parameter: [P_MONAT, P_BETRIEB],
    // Bewusst OHNE den operativ-Filter und mit LEFT JOIN auf die
    // Nachbarschaft: wer ein stillgelegtes Haus oder eines ohne
...
SELECT r.betrieb                                              AS "Betrieb",
       ...
       coalesce(s.haeuser, 0)                                 AS "Häuser mit Umsatz",
 ...
 ORDER BY coalesce(n.haeuser_am_ort, 0) DESC, n.ort NULLS LAST, r.umsatz_ist DESC NULLS LAST`,
  ...
  },
  {
    ...
    name: 'Umsatzveränderung der Nachbarhäuser',
    beschreibung:
      'Die Umsatzveränderung gegenüber dem Vorjahresmonat, ein Balken je Haus der Stadt. Die Karte, wegen der es diese Seite gibt: zeigen alle Balken nach unten, war es die Stadt — steht einer allein im Minus, war es das Haus.\n\nOhne Auswahl oben stehen hier die Städte selbst, jede als mittleres Haus.',
    anzeige: 'row',
    parameter: [P_MONAT, P_BETRIEB],
    sql: `${MONAT_CTE}
, bezug AS (
...
SELECT x."Haus", x."Umsatz % ggü. Vorjahr"
  FROM (
        ...
        UNION ALL
        SELECT s.ort || ' (' || s.haeuser::text || ' Häuser)',
               s.umsatz_pct
          FROM mart.stadt_schnitt_monat s
          CROSS JOIN gewaehlt g
 ...
 ORDER BY x."Umsatz % ggü. Vorjahr" DESC NULLS LAST`,
  ...
  },
  {
    ...
    name: 'Die Häuser der Stadt nebeneinander',
    beschreibung:
      'Alle Häuser am selben Ort mit ihren Kennzahlen. Ein ◀ markiert den oben gewählten Betrieb; ohne Auswahl stehen hier alle Städte mit mehr als einem Haus.\n\nDie **Veränderung** ist zwischen den Häusern vergleichbar, die **absoluten Quoten** nur bedingt: die Häuser gehören verschiedenen Marken mit verschiedenen Karten, Preisen und Personalstrukturen. Für den Vergleich innerhalb einer Marke ist die Seite „⑨ Betrieb gegen Marke“ da.',
    anzeige: 'table',
    parameter: [P_MONAT, P_BETRIEB],
    sql: `${MONAT_CTE}
, bezug AS (
...
SELECT ${MARKIERUNG}                              AS "◀",
 ...
 ORDER BY n.ort, r.umsatz_ist DESC NULLS LAST`,
  ...
  },
  {
    ...
    name: 'Umsatzentwicklung der Nachbarhäuser',
    beschreibung:
      'Dieselbe Frage über die Zeit: die Umsatzveränderung gegenüber dem jeweiligen Vorjahresmonat, eine Linie je Haus der Stadt, zwei Jahre bis zum gewählten Monat. Ein gemeinsamer Knick ist ein Stadtereignis — eine Baustelle, ein Umbau am Platz, eine ausgefallene Veranstaltung. Ein einzelner ist ein Hausereignis.\n\nOhne Auswahl oben stehen hier die Städte selbst.',
    anzeige: 'line',
    parameter: [P_MONAT, P_BETRIEB],
    sql: `${MONAT_CTE_UMSATZ}
, bezug AS (
...
SELECT x."Monat", x."Haus", x."Umsatz % ggü. Vorjahr"
  FROM (
        SELECT y.monat                            AS "Monat",
               y.betrieb                          AS "Haus",
               y.umsatz_pct                       AS "Umsatz % ggü. Vorjahr",
 ...
 ORDER BY x.sortierung, x."Monat"`,
  ...
  },
  {
    ...
    beschreibung:
      'Jede der sechs bewerteten Kennzahlen gegen den mittleren Wert der eigenen Stadt. „Stellung“ sagt besser oder schlechter, nicht höher oder niedriger.\n\n**Bei den Quoten mit Vorsicht zu lesen** — die Häuser einer Stadt gehören verschiedenen Marken, und ein Wareneinsatz von 24 % ist zwischen einem mexikanischen und einem bürgerlichen Konzept keine gemeinsame Messlatte. Belastbar ist die Zeile „Umsatz“: sie misst die Veränderung, die jedes Haus in seiner eigenen Einheit trägt.\n\nBleiben Vergleichswert und Rang leer, hat am Ort kein zweites Haus diese Kennzahl erfasst. Kennzahlen, für die der Betrieb selbst keinen Wert hat, stehen nicht in der Liste.',
    anzeige: 'table',
    parameter: [P_MONAT, P_BETRIEB],
    sql: `${MONAT_CTE}
SELECT v.bereich_name                             AS "Kennzahl",
 ...
 WHERE v.monat = g.monat
   ...
   -- diesem Ort ueberhaupt ein zweites Haus", nicht "hat das zweite Haus
   ...
   -- liest sich als fehlende Kennzahl im eigenen Haus.
 ...
 ORDER BY v.ort, v.reihenfolge, v.abweichung NULLS LAST`,
  },
  {
    ...
    beschreibung:
      'Betriebe ohne hinterlegten Ort. Sie tauchen in keiner Stadt auf — auch nicht in einer, in der sie tatsächlich stehen. Solange hier ein Haus mit laufendem Umsatz steht, ist mindestens eine Vergleichsgruppe unvollständig, ohne dass es dort auffiele.\n\nDie Ortsangaben werden von Hand gepflegt; das Kassensystem liefert für Betriebe keine Adresse. Erwartung: für Betriebe mit laufendem Umsatz leer.',
    anzeige: 'table',
    parameter: [P_BETRIEB],
    sql: `
