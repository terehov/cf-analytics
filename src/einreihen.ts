 ...
 *
 * Am 13.08.2026 standen hier kurzzeitig zwei weitere Schalter:
 * `--foodnotify-inventurpositionen` für die 936 abgeschnittenen
 * Inventurpositionen und `--aufgegebene` für die 275 aufgegebenen Posten.
 * Beide sind wieder verschwunden, und zwar nicht aus Bequemlichkeit:
 *
 * Eine Reparatur, die ein Mensch anstoßen muss, ist keine Reparatur, sondern
 * eine Verabredung. Sie fällt irgendwann aus, und ihr Ausfall sieht aus wie
 * Ruhe — genau daran stand LINA am 02.08.2026 acht Tage still, und genau
 * daran fror das Belegarchiv am 12.08.2026 ein. Beides ist jetzt Sache von
 * `nachfuellen()`: `inventurpositionenNachziehen()` vergleicht die Kopfzahl
 * jeder Inventur mit den geladenen Zeilen, `aufgegebeneWiederbeleben()` holt
 * aufgegebene Posten begrenzt zurück. Beide laufen bei JEDEM Sync-Lauf.
 *
 * Der Unterschied zu den Historien-Backfills ist nicht die Größe, sondern die
 * Frage, die sie beantworten. „Hol die Jahre 2018 bis 2024" ist eine
