...
| **`plan-skybridge.md`** | **Der Plan, mit dem andere die Daten selbst befragen**: ein MCP-Server ueber `mart` fuer Claude, ChatGPT und Copilot, gebaut mit Skybridge. Enthaelt die Begruendung, warum freies SQL auf `core` hier die falsche Loesung ist, die Leserolle, den Befund-Anhang und die Phasen | Bevor du am MCP-Zugang, an `mcp_leser` oder an einem Werkzeug dafuer arbeitest |
