...
| **`plan-skybridge.md`** | **Der Plan, mit dem andere die Daten selbst befragen** — Daniel mit ChatGPT, Copilot oder Claude. Stufe 0: Metabases eingebauten MCP-Server einschalten und zwei Wochen messen. Stufe 1: eigener Server (Skybridge) mit Koernung, Achsen und Fallstricken als Daten in `mcp.*`, Pruefung der Abfrage auf dem Syntaxbaum, Leserolle `mcp_leser`. Enthaelt die zehn Fallenfragen, gegen die beides gemessen wird | Bevor du am MCP-Zugang, an `mcp_leser`, an `mcp.*` oder an einer Sicht-Koernung arbeitest |
