...
| **`plan-lina-kassendaten.md`** | **Finanzwege, Rabatte, Bonebene aus LINA** — Anlass Glücksrad (Menge je Artikel und Finanzweg 3500–3502, mit unserem Bestand nicht beantwortbar). Keiner der 72 Betriebsberichte wird geladen; Erkundung, Importerumbau für `storeId`, Bonebene samt Historie, Kostenrechnung auf 152.840 Betrieb-Tage. Hypothese: CF bucht Nachlässe als Finanzweg, deshalb ist der Rabattbericht leer | Bevor du einen Betriebsbericht (`getReport`), das Kassenjournal oder Zahlungsdaten anfasst |
| **`plan-lina-vollabzug.md`** | **Alles, was aus LINA noch zu holen ist** — der Ausbau des Kassendaten-Plans auf den ganzen Katalog der 72 Betriebsberichte, damit Fragen wie das Glücksrad im Haus beantwortet werden statt per Auftrag. Enthält den **Fragenkatalog** (welcher Bericht beantwortet welche Fachfrage, in welcher Körnung, und was er ausdrücklich NICHT kann), die Einteilung in Stufe A/B/C mit Aufrufkosten auf 152.840 Betrieb-Tage, das Datenmodell (`core.finanzweg*`, `core.rabatt_artikel_tag` namensgeschlüsselt, `core.bon` mit 30,3 Mio. Zeilen), die drei Lücken im Importer samt Gegenprobe gegen `balanceSumNetto` — und den Weg bis zur Selbstbedienung über den MCP-Zugang. Zwei Berichte im Tagesraster kosten elf Wochen Backfill, jeder weitere fünfeinhalb | Bevor du einen weiteren LINA-Endpunkt aktivierst, den Betriebsbericht-Einreihweg baust oder eine Kassenfrage als „nicht beantwortbar" weitergibst |
...
| `0098_bestelldetail_nach_aenderung.sql` | **Bestelldetails nach Aenderung statt nach Frist.** `core.bestellung` traegt `listen_fingerabdruck` (md5 des Listeneintrags aus `fn:bestellungen`) und `detail_fingerabdruck` (wofuer das Detail geholt wurde); gehen sie auseinander, gibt es Arbeit. Vorher wurden 2.960 Bestellungen je Nacht doppelt geholt = 5.920 Aufrufe, davon **4.288 nachweislich ohne Ertrag**: von 408 Kopfaenderungen lagen 277 in den ersten drei Tagen, 6 bis Tag 14 und KEINE danach; bei den Positionen aenderte sich in 400 Bestellungen ueber zwoelf Tage NICHTS am Inhalt — nur der mitgelieferte Lagerbestand des Artikelstamms, den dieses Projekt gar nicht erfasst. **Deshalb kommt der Fingerabdruck aus der LISTE**, nicht aus dem Detail: `raw.payload_hash` ist dort zu 81 % Rauschen. Mitgegangen: die Pruefzeile „Details im Fenster aelter als 48 h" ist entfernt (sie beschriebe ein Verfahren, das es nicht mehr gibt), `mart.bestelldetail_stand.fenster_veraltet` misst jetzt den Fingerabdruck-Unterschied |
| `0101_lochtage_nachziehen.sql` | **Lochtage holen sich selbst nach.** `mart.umsatz_lochtag` (Karte „Tage mit Datenloch" seit 0039) traegt Zustand und Zaehler wie die Nulltage aus 0100; `lochtageNachziehen()` reiht jeden faelligen Tag fuer ALLE Konzern-Tagesberichte neu ein, auch den Artikelverkauf. Anlass: 20.–22.07.2026, am 26.07. einmal zu frueh geholt (LINA fuellt erst nach 5–7 Tagen), vom Fenster ab 02.08. nicht mehr erreicht — der 22.07. stand sieben Wochen bei allen 141 Betrieben auf null, in BEIDEN Berichten, und keine Nachholung sah ihn, weil beide am Artikelverkauf als Orakel hingen |
| `0106_datenstand_artikeltage_materialisiert.sql` | **Der Datenstand zaehlt die Artikeltage nicht mehr bei jedem Aufruf.** `mart.datenstand` rechnete je Betrieb `count(DISTINCT geschaeftstag)` ueber alle 108 Artikel-Partitionen — 10 von 10,3 Sekunden, und der MCP-Zugang haengt die Sicht an JEDE Antwort (das Werkzeug `datenstand` lief am 15.09.2026 in die 20-s-Grenze). Jetzt liest sie `mart.artikeltage_basis`, aufgefrischt im Round-Table-Nachlauf: 0,3 s, Ergebnis zeilengleich. **Nur die Artikeltage sind materialisiert** — alles an `current_date` bleibt live, sonst froere nach einer ausgefallenen Nacht genau die Warnung ein, die sie melden soll |
| `0112_verkaufsstellen.sql` | **Die Spalte, die aussah, als waere sie versorgt.** `core.umsatzbericht_tag.verkaufsstelle_key` stand seit `0003` im Schema und war nie gefuellt; sieben Registereintraege `getUmsatzbericht:vs_*` holen jetzt je Verkaufsstelle (Meilenstein M0 aus `docs/plan-lina-vollabzug.md`). `mart.hauptsparte_abdeckung` haette jede Stellenzeile als zweiten Gesamtumsatz gezaehlt und ist repariert. **Der Parameter ist ungeprueft** — `mart.verkaufsstelle_abdeckung` sagt nach der ersten Nacht, ob die Summe der Stellen den Gesamtumsatz trifft |
...
| `0117_betriebsberichte_auswertung.sql` | **Die Kassendaten in `mart`.** 31 Sichten auf die Betriebsberichte (Nachlass je Artikel und Aktion, Finanzwege/Zahlarten, Bons, Storno mit Grund, Kellner, Betriebs- und Verkaufsstellen, Zeitzonen, Debitoren, Tische, Ladestand), jede für `mcp_leser` lesbar — vorher lag alles nur in `core`, also für MCP und Karten unerreichbar. Je Betrieb und Tag **eine** Finanzwegquelle (97 vor 88), Aktionen über `aktion` + `prozentsatz` statt über Nummern, Vorzeichen positiv. Zwei Materialisierungen (`finanzweg_monat_basis`, `betriebsbericht_ladestand_basis`, Refresh in Phase B **und** nach Phase C) und Ausdrucksindizes auf den Monat — gemessen in Produktionsgröße: alle typischen MCP-Abfragen unter 1 s, vorher bis 13,9 s. Abnahme: Glücksrad 149 / 1.413 / 7.335 als `mcp_leser` |
| `0118_betriebsberichte_fallstricke.sql` | **Die Fallen der Kassendaten im MCP-Prüfer.** 14 Fallstricke, drei neue Regelarten (`filter_ueber_spalte`, `wert_muster`, `sichten_mischen`) — Glücksrad über eine Nummernliste ist gesperrt, Glücksrad in den Bons auch (96 führt keine Nachlässe), Artikel gegen Vorgänge wird gewarnt. Ohne die Umsetzung in `mcp/src/pruefen.ts` startet der Server nicht |
| `0119_bericht_88_abgeschaltet.sql` | **Bericht 88 (Finanzwege) abgeschaltet**, die Finanzwege kommen nur noch aus 97 (Tagesabschluss) — drei Stichproben identisch bis Januar 2019, rund 153.000 Aufrufe Historie gespart. Neues Warteschlangen-Ergebnis `abgeschaltet` (nicht `aufgegeben`: das würde wiederbelebt); `sync.quelle` führt 88 als `erwartet = false` mit Begründung. **Abschalten heißt `aktiv: false`, nicht löschen** — der Lader bleibt, `core` muss aus `raw` neu aufbaubar sein. ~~Preis: die Finanzwege eines Monats stehen erst ab Monatsende + 7 Tagen da~~ (behoben mit `0120`) |
| `0120_laufender_monat_vorlaeufig.sql` | **97 jede Nacht für den laufenden Monat bis zum Vortag**, als Tagesgeschäft — die Finanzwege hängen nicht mehr fünf Wochen hinterher. `core.betriebsbericht_abruf.vorlaeufig`: vor der Reife geholt, die Gegenprobe prüft es nicht, der Ladestand sagt „teilweise", der Einreihweg zieht es nach, bis es endgültig ist. **Ein Abruf ersetzt jede Abrufzeile, deren Zeitraum er enthält** — sonst 30 Teilmonatszeilen je Betrieb und Monat |
| `0121_gegenprobe_und_ladestand_genauer.sql` | **Die Gegenprobe kennt das eigene Loch, der Ladestand den Tag.** `befund = 'umsatzbericht lueckenhaft'`, wenn LINAs Summe über unserem Umsatzbericht liegt und der Zeitraum einen Lochtag oder Nulltag enthält — kein Nachholen (21./22.07.2026: ~1.100 Abrufe, ~2.800 Aufrufe gespart). Der Ladestand-Satz nennt bei Tagesberichten den Tag statt des Monats, trennt vorläufig / teilweise / nicht geladen und sagt, wann die Materialisierung zuletzt aufgefrischt wurde — sie kann während Phase C einen halben Tag alt sein |
...
cd mcp && bun test          # 386 Tests: Fallenfragen, Umgehungen aus dem Review, Anmeldeablauf, Spaltenprofil, Zugriffsprotokoll
cd mcp && bun run nutzer liste    # Nutzer verwalten (anlegen, stufe, passwort, sperren)
cd mcp && bun run start     # braucht MCP_DATABASE_URL, MCP_AUTH_DATABASE_URL und MCP_OEFFENTLICHE_URL (mit https://)
...
-- Lochtage: fast kein Betrieb meldet Umsatz, BEIDE Berichte leer (seit 0101). ERWARTUNG:
-- nichts ausser 'im Fenster'; 'aufgegeben' heisst: dreimal nachgeholt, LINA hat den Tag nicht.
SELECT * FROM mart.umsatz_lochtag WHERE zustand <> 'im Fenster';
